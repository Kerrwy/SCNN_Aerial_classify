...
from spiking_model import get_log_keys, get_plot_keys, get_type, echo
...
class AbstractSNN:
    ...
    def __init__(self, config, queue=None):
        ...
        self.batch_size = self.adjust_batchsize()   # From config batch_size
    ...
    def build(self, parsed_model, weights):
        ...
        for layer in parsed_model.layers[1:]:
            ...
            if layer_type == 'Dense':
                ...
                self.build_convolution(layer)   # no use
    ...
    def run(self, x_test=None, y_test=None, dataflow=None, **kwargs):
        ...
        path_acc = os.path.join(os.getcwd(), 'accuracy.txt')
        ...
        for batch_idx in range(num_batches):
            ...
            #     snn_plt.plot_error_vs_time(
            #         self.top1err_b_t, self.top5err_b_t, self._duration,
            #         self._dt, self.top1err_ann, self.top5err_ann,
            #         log_dir)        # I am add
            #
        ...
        #     if gt == p:
        #         match[gt] += 1
        # avg_acc = np.mean(np.true_divide(match, count))  # how many times equal
...
def initialize_simulator(config):
    ...
    import temporal_mean_rate_tensorflow as sim
