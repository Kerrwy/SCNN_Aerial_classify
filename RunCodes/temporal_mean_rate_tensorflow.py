...
class SpikeLayer(Layer):
    ...
    def __init__(self, **kwargs):
        ...
        import os
        from spiking_model import get_abs_path
        path, filename = \
        ...
        self.online_normalization = self.config.getboolean(
            'normalization', 'online_normalization')  # False
    ...
    def softmax_activation(self, mem):
        ...
        # activ = k.cast(k.softmax(mem), k.floatx())
        # max_activ = k.max(activ, axis=1, keepdims=True)
        # output_spikes = k.tf.where(k.equal(activ, max_activ), k.ones_like(activ), k.zeros_like(activ))
        # return output_spikes       # k.random_uniform(k.shape(mem)):[[0.56818247 0.83019626 0.95223606 0.1466322  0.14619708 0.34904623 0.92120314 0.7395576 ]]  it is random
    ...
    def set_reset_mem(self, mem, spikes):
        ...
        if hasattr(self, 'activation_str') and self.activation_str == 'softmax':
            ...
            if self.payloads:  # Experimental.
                ...
                # new = k.tf.where(k.less(spikes, 0), new + self.v_thresh, new)     # I am comment
    ...
    def init_neurons(self, input_shape):
        ...
        from spiking_model import get_log_keys, get_plot_keys
        ...
        if self.tau_refrac > 0:   # self.tau_refrac： 0.0
...
def spike_call(call):
    def decorator(self, x):
        ...
        self.impulse = call(self, x)    # same with z in matlab
        return self.update_neurons()  # after f(w*x+b)

        # if self.__class__.__name__ == 'SpikeDense':
        #     return self.update_neurons()    # after f(w*x+b)
        # return self.impulse
